...
//===----------------------------------------------------------------------===//
///
/// Implements the object specific parts of reading the dylib files.
///
//===----------------------------------------------------------------------===//
...
Error readSymbols(MachOObjectFile *Obj, RecordsSlice &Slice,
  ...
  for (const auto &Sym : Obj->symbols()) {
    ...
    if (Opt.Undefineds && (Flags & SymbolRef::SF_Undefined))
      ...
      assert(Exp != Exports.end() &&
             "expected export found in n-list to be in export trie");
